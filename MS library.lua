...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        New("UIPadding", {
            PaddingLeft = UDim.new(0, 8),
            PaddingRight = UDim.new(0, 32), -- 给搜索图标留空间
            Parent = SearchBox,
        })
        New("UIStroke", {
            Color = "OutlineColor",
        ...
        -- 搜索框高度加上边距
        local searchOffset = 40 * Library.DPIScale
        ...
        do
            ...
            TabLeft.Position = UDim2.fromOffset(0, searchOffset)
            TabLeft.Size = UDim2.new(0, math.floor(TabContainer.AbsoluteSize.X / 2) - 3, 1, -searchOffset)
        ...
        TabRight = New("ScrollingFrame", {
            ...
            BackgroundTransparency = 1,
            CanvasSize = UDim2.fromScale(0, 0),
            Position = UDim2.new(1, 0, 0, searchOffset),
            ScrollBarThickness = 0,
            Parent = TabContainer,
        })
        New("UIListLayout", {
        ...
        do
            ...
            TabRight.Size = UDim2.new(0, math.floor(TabContainer.AbsoluteSize.X / 2) - 3, 1, -searchOffset)
    ...
    local Tab = {
        ...
        AllElements = {},  -- 新增：存储所有元素信息
        SearchResults = {},  -- 新增：存储搜索结果
        ElementOrderMap = {},  -- 新增：存储元素顺序映射
    }

    --// 新增：搜索功能实现 \\--
    local function performSearch()
        local searchText = SearchBox.Text:lower():gsub("%s+", "")
        
        if searchText == "" then
            -- 清空搜索，恢复所有元素的原始状态
            for elementId, elementData in pairs(Tab.AllElements) do
                local element = elementData.element
                
                -- 恢复原始可见性
                if element.Holder then
                    element.Holder.Visible = elementData.originalVisible
                end
                
                -- 恢复原始顺序
                if element.Holder and elementData.originalOrder then
                    element.Holder.LayoutOrder = elementData.originalOrder
                end
            end
            
            -- 恢复所有Groupbox和Tabbox的原始状态
            for _, groupbox in pairs(Tab.Groupboxes) do
        ...
        -- 搜索Groupbox中的元素
        for groupboxName, groupbox in pairs(Tab.Groupboxes) do
            local groupboxHasMatch = false
            
            for _, element in pairs(groupbox.Elements) do
                if element.Text and element.Text:lower():find(searchText, 1, true) then
                    -- 找到匹配的元素
                    groupboxHasMatch = true
                    elementCount = elementCount + 1
                    
                    -- 存储匹配的元素信息
                    table.insert(matchedElements, {
                        element = element,
                        order = elementCount,
                        groupbox = groupbox
                    })
                    ...
                    if element.Holder then
                        ...
                        element.Holder.LayoutOrder = elementCount
                    end
                else
                    -- 隐藏不匹配的元素
                    if element.Holder then
            ...
            -- 控制Groupbox的可见性
            groupbox.Holder.Visible = groupboxHasMatch
            if groupboxHasMatch then
                groupbox:Resize()
            end
        end
        ...
        -- 搜索Tabbox中的元素
        for _, tabbox in pairs(Tab.Tabboxes) do
            local tabboxHasMatch = false
            
            for tabName, subtab in pairs(tabbox.Tabs) do
                local subtabHasMatch = false
                ...
                -- 控制Subtab按钮的可见性
                subtab.ButtonHolder.Visible = subtabHasMatch
                
                -- 如果是当前激活的Tab且有匹配，调整大小
                if tabbox.ActiveTab == subtab and subtabHasMatch then
                    subtab:Resize()
                end
            end
            
            -- 控制Tabbox的可见性
            tabbox.Holder.Visible = tabboxHasMatch
        end
        
        -- 存储搜索结果
        Tab.SearchResults = matchedElements
    end
    ...
    function Tab:Resize(ResizeWarningBox: boolean?)
        ...
        local searchOffset = 40 * Library.DPIScale
        local warningOffset = WarningBox.Visible and WarningBox.AbsoluteSize.Y + 6 or 0
        local totalOffset = searchOffset + warningOffset
        
        for _, Side in pairs(Tab.Sides) do
            Side.Position = UDim2.new(Side.Position.X.Scale, 0, 0, totalOffset)
            Side.Size = UDim2.new(0, math.floor(TabContainer.AbsoluteSize.X / 2) - 3, 1, -totalOffset)
    ...
    function Tab:AddGroupbox(Info)
        ...
        -- 重写 AddLabel
        originalMethods.AddLabel = Groupbox.AddLabel
        function Groupbox:AddLabel(...)
            local label = originalMethods.AddLabel(self, ...)
            
            -- 记录元素的原始状态
            if label.Holder then
                Tab.AllElements[tostring(label.Holder)] = {
                    element = label,
                    originalVisible = label.Visible ~= false,
                    originalOrder = label.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
        ...
        -- 重写 AddButton
        originalMethods.AddButton = Groupbox.AddButton
        function Groupbox:AddButton(...)
            local button = originalMethods.AddButton(self, ...)
            
            -- 记录元素的原始状态
            if button.Holder then
                Tab.AllElements[tostring(button.Holder)] = {
                    element = button,
                    originalVisible = button.Visible ~= false,
                    originalOrder = button.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
        ...
        -- 重写 AddToggle
        originalMethods.AddToggle = Groupbox.AddToggle
        function Groupbox:AddToggle(Idx, Info)
            local toggle = originalMethods.AddToggle(self, Idx, Info)
            
            -- 记录元素的原始状态
            if toggle.Holder then
                Tab.AllElements[tostring(toggle.Holder)] = {
                    element = toggle,
                    originalVisible = toggle.Visible ~= false,
                    originalOrder = toggle.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
        ...
        -- 重写 AddCheckbox
        originalMethods.AddCheckbox = Groupbox.AddCheckbox
        function Groupbox:AddCheckbox(Idx, Info)
            local checkbox = originalMethods.AddCheckbox(self, Idx, Info)
            
            -- 记录元素的原始状态
            if checkbox.Holder then
                Tab.AllElements[tostring(checkbox.Holder)] = {
                    element = checkbox,
                    originalVisible = checkbox.Visible ~= false,
                    originalOrder = checkbox.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
        ...
        -- 重写 AddInput
        originalMethods.AddInput = Groupbox.AddInput
        function Groupbox:AddInput(Idx, Info)
            local input = originalMethods.AddInput(self, Idx, Info)
            
            -- 记录元素的原始状态
            if input.Holder then
                Tab.AllElements[tostring(input.Holder)] = {
                    element = input,
                    originalVisible = input.Visible ~= false,
                    originalOrder = input.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
        ...
        -- 重写 AddSlider
        originalMethods.AddSlider = Groupbox.AddSlider
        function Groupbox:AddSlider(Idx, Info)
            local slider = originalMethods.AddSlider(self, Idx, Info)
            
            -- 记录元素的原始状态
            if slider.Holder then
                Tab.AllElements[tostring(slider.Holder)] = {
                    element = slider,
                    originalVisible = slider.Visible ~= false,
                    originalOrder = slider.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
        ...
        -- 重写 AddDropdown
        originalMethods.AddDropdown = Groupbox.AddDropdown
        function Groupbox:AddDropdown(Idx, Info)
            local dropdown = originalMethods.AddDropdown(self, Idx, Info)
            
            -- 记录元素的原始状态
            if dropdown.Holder then
                Tab.AllElements[tostring(dropdown.Holder)] = {
                    element = dropdown,
                    originalVisible = dropdown.Visible ~= false,
                    originalOrder = dropdown.Holder.LayoutOrder or #Tab.AllElements + 1
                }
            end
    ...
    function Tab:AddTabbox(Info)
        ...
        function Tabbox:AddTab(Name)
            ...
            --// 新增：重写SubTab的添加元素方法 \\--
            local originalSubTabMethods = {}
            
            -- 重写 AddLabel
            originalSubTabMethods.AddLabel = SubTab.AddLabel
            function SubTab:AddLabel(...)
                local label = originalSubTabMethods.AddLabel(self, ...)
