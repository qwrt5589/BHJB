...
---------------分割线-------------------------------分割线-------------------------------分割线----------------
        ...
        function tab.section(tab, name, TabVal)
          ...
          -- 添加搜索功能到第一个页面
          if Tab == TabMain:FindFirstChild("Tab") then
            -- 在第一个section前添加搜索栏
            local SearchModule = Instance.new("Frame")
            local SearchBack = Instance.new("TextButton")
            local SearchBackC = Instance.new("UICorner")
            local SearchIcon = Instance.new("ImageLabel")
            local SearchBox = Instance.new("TextBox")
            
            SearchModule.Name = "SearchModule"
            SearchModule.Parent = Objs
            SearchModule.BackgroundColor3 = Color3.fromRGB(139,0,255)
            SearchModule.BackgroundTransparency = 1.000
            SearchModule.BorderSizePixel = 0
            SearchModule.Position = UDim2.new(0, 0, 0, 0)
            SearchModule.Size = UDim2.new(0, 428, 0, 38)
            SearchModule.LayoutOrder = -100 -- 确保搜索栏在顶部
            
            SearchBack.Name = "SearchBack"
            SearchBack.Parent = SearchModule
            SearchBack.BackgroundColor3 = zyColor
            SearchBack.BackgroundTransparency = ALTransparency
            SearchBack.BorderSizePixel = 0
            SearchBack.Size = UDim2.new(0, 428, 0, 38)
            SearchBack.AutoButtonColor = false
            SearchBack.Font = Enum.Font.GothamSemibold
            SearchBack.Text = ""
            SearchBack.TextColor3 = ALcolor
            SearchBack.TextSize = 16.000
            
            SearchBackC.CornerRadius = UDim.new(0, 6)
            SearchBackC.Name = "SearchBackC"
            SearchBackC.Parent = SearchBack
            
            SearchIcon.Name = "SearchIcon"
            SearchIcon.Parent = SearchBack
            SearchIcon.BackgroundTransparency = 1
            SearchIcon.BorderSizePixel = 0
            SearchIcon.Position = UDim2.new(0.02, 0, 0.2, 0)
            SearchIcon.Size = UDim2.new(0, 24, 0, 24)
            SearchIcon.Image = "rbxassetid://117316662967569" -- 搜索图标
            SearchIcon.ImageColor3 = ALcolor
            
            SearchBox.Name = "SearchBox"
            SearchBox.Parent = SearchBack
            SearchBox.BackgroundColor3 = Color3.fromRGB(139,0,255)
            ...
            SearchBox.PlaceholderColor3 = ALcolor
            SearchBox.Text = ""
            SearchBox.TextColor3 = ALcolor
            ...
            SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
                ...
                for _, element in ipairs(allElements) do
                    if not element.originalOrder then
                        element.originalOrder = element.Module.LayoutOrder
                    end
                end
                ...
                if searchText == "" then
                    -- 恢复原始顺序
                    for _, element in ipairs(allElements) do
                        if element.originalOrder then
                            element.Module.LayoutOrder = element.originalOrder
                        end
                        element.Module.Visible = true
                    end
                else
                    -- 搜索匹配的元素
                    local matchedElements = {}
                    local unmatchedElements = {}
                    
                    for _, element in ipairs(allElements) do
                        if element.Text:lower():find(searchText) then
                            table.insert(matchedElements, element)
                            element.Module.Visible = true
                        else
                            table.insert(unmatchedElements, element)
                            element.Module.Visible = false
                        end
                    end
                    
                    -- 将匹配的元素置顶
                    for i, element in ipairs(matchedElements) do
                        element.Module.LayoutOrder = i
                    end
                    
                    -- 将不匹配的元素放在后面
                    for i, element in ipairs(unmatchedElements) do
                        element.Module.LayoutOrder = #matchedElements + i
                    end
                end
            end)
          end
          ...
          function section.Keybind(section, text, default, callback)
            ...
            table.insert(allElements, {
                ...
                Object = {
                    SetKey = function(key)
                        bindKey = key
                        KeybindValue.Text = shortNames[key.Name] or key.Name
                    end
                }
            })
          end
