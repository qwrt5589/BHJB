...
local function loadWindUIRemote()
    local url = "https://raw.githubusercontent.com/qwrt5589/BHJB/de6f3075dcbbf696d615b647d1b675f05aef4395/%E8%BE%B0ui.txt"
    print("正在从GitHub加载WindUI库...")
    local success, result = pcall(function()
        ...
        if content and #content > 0 then
            ...
            if func then
                ...
                error("loadstring失败：获取的内容不是有效Lua代码")
            end
        else
            error("HttpGet失败：获取内容为空")
        end
    end)
    if success then
        print("WindUI库加载成功")
...
local function loadWindUILocal()
    print("使用本地备用WindUI库...")
    local LocalWindUI = {}
    function LocalWindUI:CreateWindow(config)
        print("创建窗口:", config.Title)
        local mockWindow = {
            Title = config.Title or "默认窗口",
            UIElements = {
                Main = {
                    Name = "MainFrame"
                }
        ...
        function mockWindow:Tag(tagConfig)
            print("添加标签:", tagConfig.Title)
            return {
                Title = tagConfig.Title
            }
        end
        function mockWindow:Tab(tabConfig)
            print("创建标签页:", tabConfig.Title)
            return {
    ...
    function LocalWindUI:Notify(notifyConfig)
        print("显示通知:", notifyConfig.Title, "-", notifyConfig.Content)
    end
    return LocalWindUI
end

WindUI = loadWindUIRemote() or loadWindUILocal()
