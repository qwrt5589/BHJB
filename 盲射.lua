    ...
    Callback = function(Value)
        ...
        if not Value then
            for _, p in pairs(Players:GetPlayers()) do
                if p.Character and p.Character:FindFirstChild("VNGEx_Highlight") then
                    p.Character.VNGEx_Highlight:Destroy()
                end
            end
        end
    end,
...
RunService.RenderStepped:Connect(function()
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if BlindShot_ESP then
                if not char:FindFirstChild("VNGEx_Highlight") then
                    local hl = Instance.new("Highlight")
                    hl.Name = "VNGEx_Highlight"
                    hl.Parent = char
    ...
    if BlindShot_Aimbot and game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
        local target = nil
